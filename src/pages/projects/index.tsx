...
function ProjectListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface[]>(
    ...
    () =>
      getProjects({
        relations: ['user_project_artist_idTouser', 'user_project_admin_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_project_artist_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_project_admin_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_project_artist_idTouser?.id}`}>
                          {record.user_project_artist_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_project_admin_idTouser?.id}`}>
                          {record.user_project_admin_idTouser?.email}
                        </Link>
                      </Td>
                    )}
